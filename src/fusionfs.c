...
int fusion_readlink(const char *path, char *link, size_t size) {
	...
	//spade_readlink(fpath, fuse_get_context()->pid, iotime);
...
int fusion_unlink(const char *path)
{
	...
	//spade_unlink(fpath, fuse_get_context()->pid);
...
int fusion_symlink(const char *path, const char *link) {
	...
	//spade_symlink(path, flink, fuse_get_context()->pid);
...
int fusion_rename(const char *path, const char *newpath) {
    ...
    //spade_rename(fpath, fnewpath, fuse_get_context()->pid, 0, iotime);

    log_msg("\n cshou debug: returned from spade_rename.\n\n");
...
int fusion_link(const char *path, const char *newpath) {
	...
	//spade_link(fpath, fnewpath, fuse_get_context()->pid);
...
int fusion_write(const char *path, const char *buf, size_t size, off_t offset,
    ...
    spade_write(myaddr, fpath, fuse_get_context()->pid, iotime, 0);
...
int fusion_flush(const char *path, struct fuse_file_info *fi)
{
	...
	log_msg("=====cshou debug fusion_flush: can we get pid here ? %d\n", pid);
	...
	if (O_ACCMODE & flags) {
		iswritten = 1;
		log_msg("=======cshou debug: file is written\n");
	}
	...
	if (iswritten) {
		...
		spade_push(myip, fpath, pid, 1);
	}
	else {
		// spade_read(myip, fpath, pid, 0, 0);
		spade_push(myip, fpath, pid, 0);
	}
